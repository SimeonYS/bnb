...
class BnbPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bnb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bnb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bnb (date, title, link, content)"
